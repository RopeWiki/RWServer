...
public:
	...
	CANISCIOLTI(const char *base) : BETAC(base)
	{
		umain = "http://www.canisciolti.biz/2012/canyon/indicecanyon.html";
...
int DownloadPage(const char *url, CSym &sym) 
{
	...
	*/
	...
	//GetSummary(sym, stripHTML(ExtractString(f.memory, "Difficoltà:", "", "<br")));
	sym.SetStr(ITEM_SEASON, stripHTML(ExtractString(f.memory, ">Periodo", "", "</tr")));

	vars rapmemory = ExtractString(f.memory, ">Numero calate", "", "</tr");
	sym.SetNum(ITEM_RAPS, CDATA::GetNum(stripHTML(ExtractString(rapmemory, "Calate", ">", "<"))));
	sym.SetStr(ITEM_LONGEST, GetMetric(stripHTML(ExtractString(rapmemory, " alta", ">", "<"))));

	sym.SetStr(ITEM_LENGTH, GetMetric(stripHTML(ExtractString(f.memory, "Sviluppo", ">", "<"))));
	sym.SetStr(ITEM_DEPTH, GetMetric(stripHTML(ExtractString(f.memory, "Dislivello", ">", "<"))));
	
	sym.SetStr(ITEM_SHUTTLE, stripHTML(ExtractString(f.memory, "Navetta", "", "<br").replace(" ","").replace("Circa","").replace("Possibile","Optional ").replace("Si","Yes").replace("---","").replace(",",".")));

	vars interest = ExtractString(f.memory, ">Interesse", "", "<tr");
	int stars = 0, cnt = 1;
	if (strstr(interest, "/gra6.gif"))
		stars = 5, cnt=3;
	else if (strstr(interest, "/gra5.gif"))
		stars = 5;
	else if (strstr(interest, "/gra4.gif"))
		stars = 4;
	else if (strstr(interest, "/gra3.gif"))
		stars = 3;
	else if (strstr(interest, "/gra2.gif"))
		stars = 2;
	else if (strstr(interest, "/gra1.gif"))
		stars = 1;
	else
		Log(LOGERR, "Unknown interest '%s' for %s", interest, url);
	if (stars>0)
		sym.SetStr(ITEM_STARS, starstr(stars,cnt));

	vara times;
	vars timememory = ExtractString(f.memory, ">Tempi", "", "</tr");
	const char *ids[] = { "Avvicinamento", "Progressione", "Ritorno" };
	//ASSERT( !strstr(url,"/kanion.php?id=41"));
	for (int t=0; t<sizeof(ids)/sizeof(*ids); ++t)
		{
		CString time = stripHTML(ExtractString(timememory, ids[t], "", "<br"));
		while (!ExtractStringDel(time, "(", "", ")").IsEmpty());
		times.push(vars(time).replace("ore", "h").replace("giorni", "j").replace(";","."));	
		}
	GetTotalTime(sym, times, url);
...
int DownloadBeta(CSymList &symlist) 
{
	...
	DownloadFile f;
	CString url = "http://www.canisciolti.biz/2012/canyon/indicecanyon.html";
	...
	for (int r=1; r<rlist.length(); ++r)
	  {
	  vara list(rlist[r], "href=\"shede");
	  vars newregion = stripHTML(ExtractString(list[0], "", "", "</b>"));
	  if (!newregion.IsEmpty())
		  region = newregion;
	  for (int i=1; i<list.length(); ++i)
		{
		const char *memory = list[i];
		vars link = UTF8(ExtractString(memory, "", "", "\"")); 
		vars name = UTF8(stripHTML(ExtractString(memory, "", ">", "</a>")));
		if (link.IsEmpty() || strstr(link, "legenda.html"))
			continue;

		if (link.Replace("+", "%2B"))
			link = link;
		CSym sym(urlstr("http://www.canisciolti.biz/2012/canyon/shede"+link), name);
		sym.SetStr(ITEM_REGION, region);
		printf("%d/%d %d/%d   \r", r, rlist.length(), i, list.length());
		if (!UpdateCheck(symlist, sym) && MODE>-2)
			continue;
		if (DownloadPage(sym.id, sym))
			Update(symlist, sym, NULL, FALSE);
...
static Code codelist[] = {
	...
	Code(1,1,"csb", "it", "Canisciolti.biz", "Canisciolti.biz",new CANISCIOLTI( "canisciolti.biz"), "World", "1-6 -> 1-5" ), 
