...
int GeoCache::GetNoCache(const char *address, double &lat, double &lng)
{
	...
	const vars url = vars("https://maps.googleapis.com/maps/api/geocode/xml?address=") + url_encode(stripAccents(address));
	if (f.Download(url))
		return FALSE;
...
int GeoCache::Get(const char *address, double &lat, double &lng)
{
	...
	const int found = list.Find(idstr(address));
	if (found >= 0)
	{
...
vars GeoRegion::bestgeoregion(const char *data)
{
	...
	for (int i = 1; i < list.length(); ++i)
	{
		...
		{
			const double n = CDATA::GetNum(GetToken(glist[g], 1, ':'));
			if (n < 0 || n >= maxbest)
				continue;

			vars &bestn = best[static_cast<int>(n)];
...
int GeoRegion::GetRegion(CSym &sym, vara &georegions, int *forced)
{
	...
	const vars country = GetToken(georegions[0], 0, ':');
	...
	for (int r = georegions.length() - 1; r >= 0; --r)
	{
		...
		{
			const int maxlev = (int)levlist[f].GetNum(0);
...
vars GeoRegion::GetSubRegion(const char *region, const char *region2, int exact)
{
	if (exact)
	{
		...
		const int n = list.indexOfi(region2);
		return n < 0 ? "" : region2;
	}

	const vars fullreg = GetFullRegion(region);
	const char *subreg = strstri(fullreg, region2);
	return !subreg ? "" : subreg;
}

CSymList& GeoRegion::Regions()
{
...
vars GeoRegion::GetFullRegion(const char *oregion)
{
	...
	const vars region = GetRegion(GetToken(vara(oregion, ";").last(), 0, ':'));
	if (region.IsEmpty())
		return "";

	// precomputed
	CSymList &regions = Regions();
	const int or = regions.Find(region);
	if (or < 0)
		return "";
