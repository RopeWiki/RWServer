...
vars Code::Filename(int sec) const
{
...
CString Code::BetaLink(CSym &sym, vars &pre, vars &post) const
{
	CString url = sym.id;
	...
	vars pattern;
	const vars language = translink ? translink : "en";
...
BOOL Code::FindLink(const char *url)
{
	...
	const vars id = urlstr("http:" + GetToken(GetTokenRest(url, 1, ':'), 0, " ]"));
...
vars Code::Description(const char *desc)
{
	...
	const int n = words.length() - 1;
	if (n >= 0)
	{
		const int find = list.Find(words[n]);
...
vars Code::Season(const char *_oseason)
{
	...
	if (!_oseason || _oseason[0] == 0)
		return "";
	const vars oseason = stripAccentsL(_oseason);
