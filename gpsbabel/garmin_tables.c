...
static garmin_color_t gt_colors[] =
{
	...
	{ "DarkRed",		0x000060 },
	{ "DarkGreen",		0x006000 },
	{ "DarkYellow",		0x0080A0 }, //8080=>80A0 for Orange 
	{ "DarkBlue",		0x600000 },
	{ "DarkMagenta",	0x600060 },
	{ "DarkCyan",		0x808000 },
	{ "LightGray",		0xC0C0C0 },
	{ "DarkGray",		0x606060 },
	{ "Red",			0x0000F0 },
...
void colortohsv(int rgb, double *h, double *s, double *v)
{
    double      min, max, delta;

    double r  = (rgb>>0) & 0xff;
    double g  = (rgb>>8) & 0xff;
    double b  = (rgb>>16) & 0xff;
...
void colortoclab(int rgb, double *L, double *a, double *b)
{
     double ir  = (rgb>>0) & 0xff;
     double ig  = (rgb>>8) & 0xff;
     double ib  = (rgb>>16) & 0xff;
...
double cdistance(int c1, int c2)
{
#if 0
	double L1, a1, b1, L2, a2, b2;
	colortoclab(c1, &L1, &a1, &b1);
	colortoclab(c2, &L2, &a2, &b2);
	{
	double Ld = L1-L2, ad = a1-a2, bd = b1-b2;
	...
	{
	double hd = abs(h1-h2), sd = s1-s2, vd = v1-v2;
	if (hd>180) hd=360-hd;
	return sqrt(hd*hd+sd*sd/100+vd*vd/100);
	}
#endif

	/*
	double rd = (c1>>0)&0xff-(c2>>0)&0xff;
	double gd = (c1>>8)&0xff-(c2>>8)&0xff;
	double bd = (c1>>16)&0xff-(c2>>16)&0xff;
...
gt_color_index_by_rgb(const int rgb)
{
	...
	double mincdist = 1e9;
	for (i = 0; i < GT_COLORS_CT; i++)
      if (gt_colors[i].rgb!=unknown_color)
	    {
		double cdist = cdistance(rgb, gt_colors[i].rgb);
		if (cdist<mincdist)
		    {
			mini = i;
			mincdist = cdist;
