...
typedef struct { const char *url, *icon; } iconmap;
iconmap imap[] = {
{ "circle", NULL },
{ "/open-diamond.png", NULL },
{ "/flag.png", NULL },
{ "/nps-scramble.png", NULL },
{ "wht-blank", NULL },
{ "pushpin", NULL },
// starrate
{ "/Starn0.png", "Civil" },
...
// campground
{ "/tent.png", "Campground" },
{ "/campground.png", "Campground" },
// camera
{ "/nps-camera.png", "Scenic Area" },
{ "/camera.png", "Scenic Area" },
// waterfall
{ "/poi.png", "Pin, Blue" },
{ "/POI.png", "Pin, Blue" },
{ "/waterfall.png", "Pin, Blue" },
{ "url=zone", "Flag, Blue" },
{ "/zone.png", "Flag, Blue" },
//crossbones
{ "/crossbones.png", "Skull and Crossbones" },
{ "/caution.png", "Skull and Crossbones" },
//descente-canyon.com
{"/marker-canyon-depart.png", "Diamond, Green" },
{"/marker-canyon-arrivee.png", "Diamond, Red" },
{"/marker-canyon-point_interne.png", "Flag, Red" },
{"/marker-canyon-point_externe.png", "Flag, Blue" },

NULL
};

const char *mapicon(const waypoint *waypointp)
{
	const char *icon = waypointp->icon_descr;
	if (!icon)
		return NULL;

	if (strncmp(icon, "http:", 5) != 0)
		return icon;

	// map http icons
	{
	int i;
	for (i=0; imap[i].url!=NULL; ++i)
		if (strstr(icon, imap[i].url))
			return imap[i].icon;

	warning( MYNAME ": unknown icon %s\n", icon);
	return imap[0].icon;
	}
}
...
//	GEOTAG( tt_cache, 		"cache"),
	...
	// color
	{ tt_garmin_rte_color, 0, "/gpx/rte/extensions/gpxx:RouteExtension/gpxx:DisplayColor", 0UL },
	{ tt_garmin_trk_color, 0, "/gpx/trk/extensions/gpxx:TrackExtension/gpxx:DisplayColor", 0UL },
...
//TODO: implement GPX 1.1 	case tt_rte_rtept_link: 
	...
	/* Garmin Color */
	case tt_garmin_rte_color:
		rte_head->line_color.bbggrr = gt_color_value_by_name(cdatastrp);
		break;
	case tt_garmin_trk_color:
		trk_head->line_color.bbggrr = gt_color_value_by_name(cdatastrp);
		break;
	...
	const char *oname)
{
	...
	write_optional_xml_entity(ofd, indent , "sym", mapicon(waypointp));
}
...
static void
gpx_color_ext(const route_head *rte, char *hdr)
{
	if (rte->line_color.bbggrr!=-1)
	  {
	  int c = gt_color_index_by_rgb(rte->line_color.bbggrr);

	  char *color = gt_color_name(c);
	  

	  gbfprintf(ofd, "<type>%s</type><extensions><gpxx:%sExtension xmlns:gpxx=\"http://www.garmin.com/xmlschemas/GpxExtensions/v3\"><gpxx:DisplayColor>%s</gpxx:DisplayColor></gpxx:%sExtension></extensions>", color, hdr, color, hdr);
  	  }
}
...
arglist_t gpx_args[] = {
	...
	{ "gpxver", &gpx_wversion, "Target GPX version for output", 
		"1.1", ARGTYPE_STRING, ARG_NOMINMAX},
