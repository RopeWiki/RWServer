...
static int stylenum = -1;
static int stylefoldernum = -1;
int stylefolder[50];
static int stylenormal, stylei;
static struct {
	int line_color;
	const char *id;
	const char *map;
...
arglist_t kml_args[] = {
	...
	{"line_width", &opt_line_width,
         ...
         "5", ARGTYPE_INT, ARG_NOMINMAX },
	{"line_color", &opt_line_color,
         "Line color, specified in hex AABBGGRR",
         "a0606060", ARGTYPE_STRING, ARG_NOMINMAX },
...
static xg_callback wpt_name, wpt_desc, wpt_coord, wpt_icon, trk_coord, trk_color, map_url, style_start, style_color, style_icon, style_url, style_map, style_mapnormal, style_push, style_pop, wpt_time;
...
xg_tag_mapping kml_map[] = {
	...
	// color/icon
	{ map_url,		cb_cdata, 	"/Placemark/styleUrl" },
	{ trk_color, 	cb_cdata, 	"/Placemark/Style/LineStyle/color" },
	{ wpt_icon, 	cb_cdata, 	"/Placemark/Style/IconStyle/Icon/href" },

	// style
	{ style_push, 	cb_start, 	"Document" },
	//{ style_push, 	cb_start, 	"Folder" },
	{ style_pop, 	cb_end, 	"Document" },
	//{ style_pop, 	cb_end, 	"Folder" },

	{ style_start, 	cb_start, 	"/Style" }, // id=?
	{ style_color, 	cb_cdata, 	"/Style/LineStyle/color" }, // color
	{ style_icon, 	cb_cdata, 	"/Style/IconStyle/Icon/href" }, // icon

	{ style_start, 	cb_start, 	"/StyleMap" }, // id="#dir_route1" -> <StyleMap id="dir_route1">
	{ style_mapnormal, cb_cdata, "/StyleMap/Pair/key" }, // id="#dir_route10" -> <Style id="dir_route10">
	{ style_map, 	cb_cdata, 	"/StyleMap/Pair/styleUrl" }, // id="#dir_route10" -> <Style id="dir_route10">
	{ NULL, 	0, 		NULL }
};
...
void wpt_desc(const char *args, const char **unused)
{
	if (args) {
		...
		if (t)
		  {
		  char *link_url = t + 8;
		  if ((t= strstr(link_url, "\"")))
			  *t = 0;
		  waypt_add_url(wpt_tmp, xstrdup(link_url), NULL);
          if ((t=strstr(c, "&lt;")) || (t=strstr(c, "<")))
			  *t = 0;
		  }
...
void wpt_coord(const char *args, const char **attrv)
{
	...
	if ( lat==0.0 || lon==0.0)
		warning("Invalid coordinate %s", args);
	else
		wpt_tmp_queued = 1;
}
...
int findstyle(const char *args)
{
int i;
for (i=0; i<=stylenum; ++i)
	if (stricmp(args, stylemap[i].id)==0)
		return i;
return -1;
}

int styleadd(const char *name)
{
	int i = findstyle(name);
	if (i>=0) return i;

	++stylenum;
	stylemap[stylenum].id = xstrdup(name);
    stylemap[stylenum].line_color = -1;
    stylemap[stylenum].map = NULL;
	return stylenum;
}


void style_start(const char *args, const char **attrv)
{
	stylei = styleadd(attrv[1]);
	stylenormal = TRUE;
}


void style_mapnormal(const char *args, const char **attrv)
{
	stylenormal = stricmp(args, "normal")==0;
}

void style_map(const char *args, const char **attrv)
{
	if (stylenum>=0 && stylenormal)
      stylemap[stylei].map = xstrdup(args);
}


void trk_color(const char *args, const char **attrv)
{
	if (sscanf(args, "%lx", &line_color) != 1)
		line_color = -1;
	line_color &= 0xFFFFFF;
}

void style_color(const char *args, const char **attrv)
{
	trk_color(args, attrv);
	if (stylenum>=0)
      stylemap[stylei].line_color = line_color;
}




void style_icon(const char *args, const char **attrv)
{
	wpt_icon(args, attrv);
	if (stylenum>=0)
	  stylemap[stylei].map = xstrdup(args);
...
static void map_color(const route_head *ctrk_head)
{
	route_head *trk_head = (route_head *)ctrk_head;
    if (trk_head->line_color.bbggrr ==-1)
		{
		// invalid
		warning(MYNAME " unknown line color for \"%s\"\n", trk_head->rte_name);
		return;
		}

	if (trk_head->line_color.bbggrr & MAPMASK)
	  {
	    // mapped
  	    const char *map ="(null)";
	    int i = trk_head->line_color.bbggrr & ~MAPMASK;

	    // map
	    while (i>=0 && stylemap[i].line_color==-1 && stylemap[i].map!=NULL)
		   i = findstyle(map = stylemap[i].map+1);

		if (i>=0 && stylemap[i].line_color!=-1 && !(stylemap[i].line_color & MAPMASK))
		   {
		   trk_head->line_color.bbggrr  = stylemap[i].line_color;
		   return;
		   }

	   warning(MYNAME " unknown style map for\"%s\" (\"%s\")\n", trk_head->rte_name, map);
	   return;
	   }

	// color int
	trk_head->line_color.bbggrr  &= 0xFFFFFF; 
}


static void map_icon(const waypoint *cwpt)
{
	waypoint *wpt = (waypoint *)cwpt;
	const char *map = wpt->icon_descr;
	while (map && *map=='#')
	   {
	   int i = findstyle(map+1);
	   
	   if (i<0) 
	     {
	     // error
	     warning(MYNAME " unknown icon style for \"%s\" (%s)\n", wpt->shortname, wpt->icon_descr);
	     return;
	     }
    ...
    // found it!
	if (map!=wpt->icon_descr)
	    {
		xfree(wpt->icon_descr);
	    wpt->icon_descr = xstrdup(map);
        }
}

void styledel(int stylemin)
{
	// map colors at the end
	track_disp_all(map_color, NULL, NULL);
	waypt_disp_all(map_icon);

	{
	int i = 0;
	for (i=stylemin+1; i<=stylenum; ++i)
	    {
		if (stylemap[i].id) xfree(stylemap[i].id);
		if (stylemap[i].map) xfree(stylemap[i].map);
	    }
	stylenum = stylemin;
	}
}

void style_push(const char *args, const char **attrv)
{
	stylefolder[stylefoldernum++] = stylenum;
}

void style_pop(const char *args, const char **attrv)
{
	if (stylefoldernum>=0)
	  styledel(stylefolder[--stylefoldernum]);
}

void map_url(const char *args, const char **attrv)
{
	int i = styleadd(args+1);
	line_color = i | MAPMASK; // codename
...
void trk_coord(const char *args, const char **attrv)
{
	...
	trk_head->line_color.bbggrr  = line_color;

	track_add_head(trk_head);

	while (*args!=0 && isspace(*args)) ++args;
	while ((n = sscanf(args, "%lf,%lf,%lf", &lon, &lat, &alt)) > 0) {
